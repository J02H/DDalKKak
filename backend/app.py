...
# Firebase 초기화
try:
    db = get_db()  # firebase_admin.db 모듈을 돌려준다고 가정
    print("✅ Firebase 연결 성공")
except Exception as e:
    print(f"❌ Firebase 연결 실패: {e}")
    print("💡 Firebase 없이 로컬 데이터로 실행합니다.")
    db = None
...
# =========================
# 🔥 Firebase Realtime DB 헬퍼
# =========================

def create_user_in_db(username: str, user_data: dict) -> bool:
    """Realtime DB: /users/{username} 에 사용자 저장"""
    if not db:
        return False
    try:
        user_ref = db.reference(f"users/{username}")
        user_ref.set(user_data)
        print(f"✅ Firebase에 사용자 {username} 저장 성공")
        return True
    except Exception as e:
...
def add_bookmark_to_db(username: str, bookmark: dict) -> bool:
    """Realtime DB: /bookmarks/{username}/{bookmark_key} 에 즐겨찾기 추가"""
    if not db:
        return False
    try:
        bookmarks_ref = db.reference(f"bookmarks/{username}")
        # push() 대신 timestamp 기반 키 사용
        bookmark_key = str(int(time.time() * 1000))
        bookmarks_ref.child(bookmark_key).set({
        ...
        print(f"✅ Firebase에 즐겨찾기 추가 성공: {username}")
        return True
    except Exception as e:
...
def get_bookmarks_from_db(username: str) -> List[dict]:
    """Realtime DB: /bookmarks/{username} 조회"""
    if not db:
        return []
    try:
        bookmarks_ref = db.reference(f"bookmarks/{username}")
        bookmarks_data = bookmarks_ref.get()
        if isinstance(bookmarks_data, dict):
            return list(bookmarks_data.values())
        return []
    except Exception as e:
...
def remove_bookmark_from_db(username: str, bookmark_id: str) -> None:
    """Realtime DB: 값 안의 bookmark_id 로 해당 노드 찾아서 삭제"""
    if not db:
        return
    try:
        bookmarks_ref = db.reference(f"bookmarks/{username}")
        bookmarks_data = bookmarks_ref.get()
        if isinstance(bookmarks_data, dict):
            for key, value in bookmarks_data.items():
                if value.get("bookmark_id") == bookmark_id or value.get("id") == bookmark_id:
                    bookmarks_ref.child(key).delete()
                    print(f"✅ Firebase 즐겨찾기 삭제 성공: {username} / {bookmark_id}")
                    break
    except Exception as e:
        print(f"❌ DB 즐겨찾기 삭제 오류: {e}")
        import traceback
        traceback.print_exc()

# =========================
# 이하 공지 크롤링/검색 로직 (기존 그대로)
# =========================
...
async def register(user_data: UserRegister):
    ...
    elif user_data.username in USERS:
        raise HTTPException(status_code=400, detail="이미 사용 중인 아이디입니다.")
    ...
    # Firebase에 저장
    if db:
        create_user_in_db(user_data.username, user_profile)
...
async def login(user_data: UserLogin):
    user_from_db = get_user_from_db(user_data.username) if db else None

    if user_from_db and user_from_db.get('password') == user_data.password:
        # Firebase 사용자
...
async def get_user(request: Request):
    ...
    if current_user:
        return {"success": True, "user": current_user, "logged_in": True}
...
async def get_bookmarks(request: Request):
    ...
    bookmarks = get_bookmarks_from_db(current_user) if db else []
    if not bookmarks:
...
async def toggle_bookmark(bookmark_data: BookmarkData, request: Request):
    ...
    bookmarks = get_bookmarks_from_db(current_user) if db else USER_BOOKMARKS.get(current_user, [])
    if not bookmarks:
        bookmarks = []

    existing_bookmark = next(
        (b for b in bookmarks if b.get('bookmark_id') == bookmark_data.id or b.get('id') == bookmark_data.id),
        None
    )
    ...
    if existing_bookmark:
        # 삭제
        if db:
            remove_bookmark_from_db(current_user, bookmark_data.id)
        if current_user in USER_BOOKMARKS:
            USER_BOOKMARKS[current_user] = [
                b for b in USER_BOOKMARKS[current_user] if b.get('id') != bookmark_data.id
            ]
        ...
        if db:
            add_bookmark_to_db(current_user, bookmark)
...
async def remove_bookmark(bookmark_id: str, request: Request):
    ...
    if db:
        remove_bookmark_from_db(current_user, bookmark_id)
